...
type
  ...
  public
    function Pessoa(IdPessoa: Integer): TJSONArray;
...
function TServerMethods.Pessoa(IdPessoa: Integer): TJSONArray;
const
  SQL = 'select p.idpessoa, p.flnatureza, p.dsdocumento, p.nmprimeiro, p.nmsegundo, e.dscep, '+
        '       ei.dsuf, ei.nmcidade, ei.nmbairro, ei.nmlogradouro, ei.dscomplemento '+
        '  from pessoa p '+
        '  join endereco e on e.idpessoa = p.idpessoa '+
        '  left join endereco_integracao ei on ei.idendereco = e.idendereco ';
  SQL_WHERE = ' where p.idpessoa = :idpessoa';
var
  LObject: TJSONObject;
  LIndex: Integer;
  LQuery: TFDQuery;
begin
  ...
  try
    ...
    if IdPessoa <> 0 then
    begin
      LQuery.SQL.Text := LQuery.SQL.Text + SQL_WHERE;
    ...
    begin
      ...
      LObject.AddPair('result', 'Não há dados para exibição');
    ...
    begin
      ...
      begin
        ...
        begin
          if LQuery.Fields[LIndex].IsNull then
            LObject.AddPair(LQuery.Fields[LIndex].DisplayName, '')
          else
            LObject.AddPair(LQuery.Fields[LIndex].DisplayName, LQuery.Fields[LIndex].Value);
        end;
        Result.AddElement(LObject);
        LQuery.Next;
      end;
